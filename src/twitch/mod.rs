...
fn generate_reply(reply_str: &str, user: &str) -> Option<String> {
    if reply_str == "default" {
        Some(format!("@{}, please ask for permission before posting a link.", user))
...
>(
    ...
    if client.privmsg(channel.to_owned(), msg_contents.to_owned()).await.is_err() {
...
>(
    ...
    limiter.until_ready().await;
    if client.say(channel.to_owned(), msg_contents.to_owned()).await.is_err() {
...
>(
    ...
    limiter.until_ready().await;
    if client
        .say_in_reply_to(&(channel.to_owned(), msg_id.to_owned()), msg_contents.to_owned())
        .await
        .is_err()
    {
        if let Err(e) = client
...
>(
    ...
    let reply = format!("Allowed links in {}: {}", chan.name, chan.allow_list.join(", "));
    try_respond(
        client,
...
>(
    ...
    let logon_name;
    {
        ...
        if r.logon_name.is_none() {
            warn!("Received message PRIVMSG before GLOBALUSERSTATE: {:#?}", privmsg);
            return;
        }
                ...
                "!links" => {
                    ...
                    if let Ok((args, command)) = parse_command_links(args).await.finish() {
                        match command {
                            ...
                            "subdomains" => {
                                let mut enabled =
                                    if chan.subdomains { "enabled" } else { "not enabled" };
...
lazy_static! {
    static ref BOT_CONFIG: std::sync::RwLock<BotConfig> =
        std::sync::RwLock::new(BotConfig { logon_name: None, moderator_id: None });
}
...
pub async fn main() {
    ...
    // Limit the scope of our file reads
    {
        ...
        config_file.read_to_string(&mut config_contents).expect("Unable to read ghirahim.yaml");
    ...
    let client_secret = config["ghirahim"]["client_secret"].as_str().unwrap().to_owned();
    ...
    let metrics_config =
        twitch_irc::MetricsConfig::Enabled { constant_labels: [].into(), metrics_registry: None };
    ...
    let helix_sent = register_counter_vec!(
        Opts::new("ghirahim_helix_sent", "Number of commands sent to the Helix API."),
        &["command"]
    )
    .unwrap();
    ...
    if std::env::var("GHIRAHIM_METRICS").is_ok() {
        let bind_addr = config["metrics"]["bind_addr"].as_str().unwrap().parse().unwrap();
        let serve_future =
    ...
    let db = GhirahimDB::new(mongo_str, &redis_str).await.expect("Could not get database");
    ...
    let rest_client;
    {
        ...
        headers.insert(
            ...
            format!("Ghirahim_Bot/{}", get_ghirahim_rs_version()).parse().unwrap(),
        );
        ...
        // Set up the client
        let temp_client = reqwest::Client::builder().default_headers(headers).build().unwrap();
    ...
    let cred_check_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            let token =
                inner_creds.get_credentials().await.expect("Could not get token").token.unwrap();
